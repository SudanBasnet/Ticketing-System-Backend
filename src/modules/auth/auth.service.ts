...
const createRefreshToken = async (user: UserDocument, ip?: string, userAgent?: string, familyId?: string) => {
...
const issueTokenPair = async (user: UserDocument, res: Response, ip?: string, userAgent?: string, familyId?: string) => {
...
export const register = async (input: { name: string; email: string; password: string }, res: Response, ip?: string, userAgent?: string) => {
...
export const login = async (input: { email: string; password: string }, res: Response, ip?: string, userAgent?: string) => {
  const user = await UserModel.findOne({ email: input.email, status: { $ne: "deleted" } }).select("+passwordHash");
  if (!user || !(await user.comparePassword(input.password))) {
    throw new AppError(401, "Invalid credentials", "INVALID_CREDENTIALS");
  }
  if (user.status === "disabled") throw new AppError(403, "Account is disabled", "ACCOUNT_DISABLED");
...
export const refresh = async (refreshToken: string | undefined, res: Response, ip?: string, userAgent?: string) => {
  ...
  if (!existing) throw new AppError(401, "Invalid refresh token", "INVALID_REFRESH_TOKEN");
  ...
  if (!user || user.status !== "active") throw new AppError(401, "User not available", "USER_UNAVAILABLE");
...
export const logout = async (refreshToken: string | undefined, res: Response, ip?: string) => {
  if (refreshToken) {
...
export const verifyEmail = async (token: string) => {
  ...
  if (!record || record.expiresAt <= new Date()) throw new AppError(400, "Invalid or expired verification token", "INVALID_VERIFICATION_TOKEN");
...
export const resetPassword = async (token: string, password: string) => {
  ...
  if (!record || record.expiresAt <= new Date()) throw new AppError(400, "Invalid or expired reset token", "INVALID_RESET_TOKEN");
