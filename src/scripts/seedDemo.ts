...
const people = [["Alex Morgan", "super_admin"], ["Jordan Lee", "admin"], ["Priya Shah", "admin"], ["Mia Chen", "agent"], ["Noah Williams", "agent"], ["Sofia Patel", "agent"], ["Liam Brown", "agent"], ["Ava Wilson", "user"], ["Ethan Davis", "user"], ["Isla Taylor", "user"], ["Lucas Martin", "user"], ["Zoe Anderson", "user"]] as const;
...
const seed = async () => {
  ...
  for (let index = 0; index < 40; index += 1) {
    ...
    const status = (["open", "in_progress", "resolved", "closed"] as const)[index % 4];
  ...
  const prefix: Record<string, string> = { request: "REQ", problem: "PRB", change: "CHG", ci: "CI", knowledge: "KB" };
  for (const [type, rows] of Object.entries(data)) for (let i = 0; i < rows.length; i += 1) await (ServiceRecordModel as any).create({ type, number: `${prefix[type]}${String(i + 1).padStart(6, "0")}`, description: `${rows[i].title} demonstration record`, priority: "medium", demoKey: `clearqueue-v1-${type}-${i}`, ...rows[i] });
  for (let i = 0; i < 12; i += 1) await WorkNoteModel.create({ incidentId: incidents[i]._id, authorId: agents[i % agents.length]._id, note: `[DEMO] ${i % 2 ? "Customer update sent and next diagnostic scheduled." : "Logs reviewed; remediation is being tested."}`, isInternal: i % 3 === 0 });
...
seed().catch((error) => { console.error(error); process.exitCode = 1; }).finally(async () => { if (mongoose.connection.readyState) await disconnectDb(); });
